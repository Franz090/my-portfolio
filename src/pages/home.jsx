...
import useHomeStore from '../store/useHomeStore'; // Update the path accordingly

function HomePage() {
  const { shouldAnimate, checkAnimation } = useHomeStore();
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:mt-11 md:mt-12">
      ...
      <div className="col-span-1 flex flex-col justify-center items-center relative xl:right-[54px] lg:top-[85px]">
        ...
        <h1 className={`font-montserrat font-semibold xl:text-7xl md:text-4xl text-4xl lg:text-left md:text-[3.8rem] lg:text-7xl md:pb-4 sm:pb-2 lg:pb-5 xl:tracking-[.2em] tracking-[.2em] lg:tracking-[0.8rem] frontend-text ${shouldAnimate ? '' : 'slide-in-animation'}`}>FRONTEND</h1>
